...
# ╔═╡ cc364c22-848a-11ee-322a-5321ed8f4241
using Downloads, CSV, DataFrames, PlutoUI

# ╔═╡ c72aeea9-f56e-49f5-a2e9-31be92d593fb
using Chain

# ╔═╡ a1c35e09-adc1-433e-9b63-59304d70d575
using StatsBase

# ╔═╡ b5aad3b7-4bf8-42f1-81b8-f9de710b8e29
using Turing, LinearAlgebra

# ╔═╡ 4bc3922a-fd50-4723-b8c3-1dff01540df7
using StatsPlots

# ╔═╡ c4195bf3-a824-415a-97c5-65b288672f3c
md"# Data prep"

# ╔═╡ 52676198-4593-4987-aa6f-9e6a42d2c8fd
const LINKS = 
	"https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2023/2023-08-08/" .* ["episodes", "sauces", "seasons"] .* ".csv"


# ╔═╡ 3b3ad57f-f226-4a15-b5e6-7164d514542d
episodes, sauces, seasons = CSV.read.(Downloads.download.(LINKS), DataFrame)

# ╔═╡ c9d16279-27e1-48ac-b549-0d2a36220019
md"## Tokenizing sentences"

# ╔═╡ 806c7c45-cef5-4cad-bb8b-3105b543f70b
rgx = r"[^(?<=')(a-z|A-Z)]"

# ╔═╡ dc1b4c36-2ed7-4866-8507-70f1d300addd
df_tokens = @chain sauces begin
	rename(_, :sauce_name => :name)
	DataFrames.transform(_, :name => ByRow(x -> replace(x, rgx => " ")) => identity)
	DataFrames.transform(_, :name => ByRow(x -> replace(x, r"'s|\(|\)" => " ")) => identity)
	DataFrames.transform(_, :name => ByRow(x -> replace(x, r"\s{2,}" => " ")) => identity)
	DataFrames.transform(_, :name => ByRow(x -> replace(x, r"\s$" => "")) => identity)
	DataFrames.transform(_, :name => ByRow(x -> split(x, " ")) => :tokens)
	select(_, :name, :scoville, :tokens)
end

# ╔═╡ e2f4ce3e-73a2-4fa6-974d-4193d75904eb
boringwords = ["with", "and", "th", "is", "o", "O", "", "N", "of"]

# ╔═╡ e3ec15d1-07d3-4537-92a5-b0e4161cbd45
df_model = @chain df_tokens begin
	map(eachrow(_)) do r
		DataFrame([fill(r.scoville, length(r.tokens)), r.tokens], [:scoville, :word])
	end
	reduce(vcat, _) 
	unique(_)
	subset(_, :word => ByRow(x -> 2 ≤ sum(x .== _.word)))
	subset(_, :word => ByRow(x -> x ∉ boringwords))
end

# ╔═╡ 32ab3262-7b20-480c-a633-a74db22e9a0f
combine(groupby(df_model, :word), nrow)

# ╔═╡ be292b99-3778-4383-b4cb-3e6ceba19a35
spanishwords = ["El", "Chile", "Habanero", "Chipotle", "Adobloco", "Los", "Calientes", "Jalape", "Shaquanda", "Lengua", "de", "Fuego"]

# ╔═╡ 14982743-ba8a-4376-8422-9f7c863444fc
df_model.spanish = map(x -> x ∈ spanishwords, df_model.word)

# ╔═╡ 92fdd2e0-ba3d-4c78-b5b9-3536c04b7870
md"## Model-ready data"

# ╔═╡ d41ca1f9-c972-44f1-a768-65e23270b8b6
standardise(x) = (x .- mean(x)) ./ std(x)

# ╔═╡ d391b3c1-9092-4cec-887d-7e619320438a
begin
	Y = standardise(log.(Float64.(df_model.scoville)))
	W = Int64.(denserank(df_model.word))
	S = Int64.(df_model.spanish .+ 1)
	DataFrame([Y, W, S], [:scoville, :word, :spanish])
end

# ╔═╡ 40f50096-6656-4b35-84ea-1058f45f682c
md"# Model equations"

# ╔═╡ dc3eb5e4-cc76-4f9e-92fa-aa92a19900ae
md"### Simple model"

# ╔═╡ a1d527f9-6fa6-4b27-b749-df5695bf53f9
@model function hotones(W, Y; nW=length(unique(W)), nS=length(unique(S)))
	# Fixed effect for word
	β ~ filldist(Normal(), nW)
...
# ╔═╡ 44b828bc-6ada-4469-b222-31eddc9d4e67
md"### Hierarchical model"

# ╔═╡ beb86531-c91c-40f0-bf61-01a4f3c888b2
@model function hotones(W, S, Y; nW=length(unique(W)), nS=length(unique(S)))
	# Random intercept for language
	μS ~ Normal()
	τS ~ filldist(InverseGamma(3, 2), nS)
	zS ~ filldist(Normal(), nS)
	α = @. μS + τS * zS
...
# ╔═╡ da86ab11-5028-4047-95c6-9f30c07cb71f
md"### Functions to simulate from each model"

# ╔═╡ 656e19c8-6190-4477-befb-b1047b60446c
function simulate(params, W)
	map(params) do param
		μ = @. param.β[W]
		rand.(Normal.(μ, param.σ²))
	end
end

# ╔═╡ 24e8e723-7869-4d52-9567-e86213f1e644
function simulate(params, W, S)
	map(params) do param
		μ = @. param.α[S] + param.β[W]
		rand.(Normal.(μ, param.σ²))
	end
end

# ╔═╡ 218608cc-ad9b-4098-bd9e-60033a73345e
md"### Set up both models"

# ╔═╡ 75142a6b-4f96-46ef-82b7-45132ee35875
simple = hotones(W, Y)

# ╔═╡ 659107d1-f11c-48ed-837d-bbc70e6c7147
multi = hotones(W, S, Y)

# ╔═╡ 76b7ffa4-688a-4a5a-917b-a73162e3a5a9
begin
	prior_chains_simple = sample(simple, Prior(), 1000)
	prior_chains_multi = sample(multi, Prior(), 1000)
end;

# ╔═╡ c41e1f74-721f-4f48-95b5-2e81af980b97
θ_prior_simple = generated_quantities(simple, prior_chains_simple);

# ╔═╡ c7320fec-8c00-4921-bc98-d7b9df443afb
θ_prior_multi = generated_quantities(multi, prior_chains_multi);

# ╔═╡ 575e7a5e-5c5e-4145-b583-e42d920f5eec
begin
	priorselect = @bind priorchoice Select(["simple", "pooled"])

	m_selected = priorchoice == "simple" ? θ_prior_simple : θ_prior_multi
...
# ╔═╡ 758fdb18-b76c-4e48-958d-fc4778bd10f9
md"Showing prior predictive distribution for $priorselect model."

# ╔═╡ b109eae6-43d6-4d29-a337-dd6b58d68675
getsamples(θ, fields...) = map(x -> NamedTuple{fields}(getfield.(Ref(x), fields)), θ)

# ╔═╡ 4ca32690-3c64-40d3-a46c-158c8357e948
begin
	k = keys(m_selected[1])
	inputs = length(k) == 3 ? [W, S] : [W]
	params = vec(getsamples(m_selected, k...))
end;

# ╔═╡ 2ab37c26-8a52-4e12-b597-03e9d431c505
let priorsamples = reduce(vcat, simulate(params, inputs...))
	density(priorsamples, fillrange=0, fillalpha=0.2, legend=false)
	xlims!(-50, 50)
	title!("Prior predictive check, $priorchoice model")
end

# ╔═╡ f0708f5c-4de2-4d30-9c26-098671e6508d
md"# Sample"

# ╔═╡ 88e66ed9-ec81-49a0-8949-02325260fa48
begin 
	nchains, nsamples = 3, 500
	chains_simple = sample(simple, NUTS(1000, 0.65), MCMCThreads(), nsamples, nchains)
	chains_multi = sample(multi, NUTS(1000, 0.95), MCMCThreads(), nsamples, nchains)
end;

# ╔═╡ 9c85112e-42ea-42c3-bbc5-413b5b6344c8
describe(chains_multi)

# ╔═╡ 503c0613-0203-4837-bd26-e21e55f4d991
plot(chains_multi[:, 1:10, :])

# ╔═╡ 4b6d37aa-1a6f-4bb1-9d74-9ca70b42e5d5
md"## Chain diagnostics"

# ╔═╡ 8b058184-bfad-4b8a-811a-ad47cd91e8c3
function diagplot(var::Symbol, chains...; seq=Int64.(1:nsamples*0.01:nsamples))
	vars = getproperty.(DataFrame.([chains...]), var)

	# Rolling mean and variance
	quantities = map(vars) do var    	
    	μ = [mean(var[seq[i]:seq[i+1]]) for i in eachindex(seq[begin:end-1])]
    	σ = [std(var[seq[i]:seq[i+1]]) for i in eachindex(seq[begin:end-1])]
    	(; μ, σ)
	end

	M = [getindex.(Ref(quantity), idx) for quantity in quantities, idx in [:μ, :σ]]

	# Assemble plot
	plot(seq[begin:end-1], M[:, 1], lw=1.5, label=["simple" "multilevel"])
	plot!(seq[begin:end-1], M[2, 1] .+ M[2, 2], fillrange=M[2, 1] .- M[2, 2], fillalpha=0.2, alpha=0, c=2, label=false)
	plot!(seq[begin:end-1], M[1, 1] .+ M[1, 2], fillrange=M[1, 1] .- M[1, 2], fillalpha=0.2, alpha=0, c=2, label=false)
	xticks!(0:100:500, string.(3 .* collect(0:100:500)))
end

# ╔═╡ bf5214ce-7d48-4e99-9dcc-e52359e87a5c
begin
	p1 = diagplot(:tree_depth, chains_simple, chains_multi);
	title!("Tree depth"); ylims!(0, 10)
	p2 = diagplot(:numerical_error, chains_simple, chains_multi)
	title!("Divergent transitions"); ylims!(0, 10)
	plot!(p1, p2, size=(700,400)); xlabel!("Iteration")
end

# ╔═╡ a9977b20-e6db-4c72-b0b7-7146fa09f849
begin
	mselect = @bind m Select([chains_simple => "simple", chains_multi => "pooled"])
	md"""
	# Posterior predictive check
	
	Showing posterior predictive check for $mselect model.
	"""
end

# ╔═╡ 190affac-a732-4c11-aa12-7e1187f9d2bb
q_post_multi = generated_quantities(multi, chains_multi)

# ╔═╡ e4e6dd23-37ab-442c-be75-4fd3b64aa061
θ_post = vec(getsamples(q_post_multi, keys(q_post_multi[1])...))

# ╔═╡ de86958d-174f-4a7b-b6bd-037ed5c03515
begin 
	res = DataFrame(mean(group(m, :β)))
	res.word = sort(unique(df_model.word))
	sort(res, :mean, rev=true)
end

# ╔═╡ 243aea9e-2d36-460c-8dff-85c8029c1b12


# ╔═╡ d646af97-b97c-4037-bc2a-f0caf8227456
boots(v::AbstractArray) = rand(v, length(v))

# ╔═╡ 4417783d-57b0-409e-a3f7-776748888e2c
S1, S2 = map([1, 2]) do language
	simulate.(Ref(θ_post), boots(fill(W, 20)), language)
end

# ╔═╡ fcc3a843-2512-4bcd-b944-7b230c6aa317
begin
	density(reduce(vcat, (reduce(vcat, S1))), lw=1.5, fillrange=0, fillalpha=0.1, label="English")
	density!(reduce(vcat, (reduce(vcat, S2))), lw=1.5, fillrange=0, fillalpha=0.1, label="Spanish")
end

# ╔═╡ 66e70bc9-b684-4d6d-83d7-7331b6468591
size(reduce(vcat, (reduce(vcat, S1)))) # woops
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "aa2ea4b13c55a83a55c006329174255c21b4f3a3"
